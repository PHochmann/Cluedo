 ...
 */
uint32_t getVar(uint8_t card, uint8_t player)
{
...
static SAT_Problem* initSatProblem(const GameSettings* settings)
{
    SAT_Problem* kb = sat_initProblem();
...
#ifdef DEBUG
    printf("Knowledge base initialized with %zu variables and %zu clauses.\n", sat_getNumVariables(kb), sat_getNumClauses(kb));
#endif
    return kb;
}
...
void kb_addGuessAnswerClauses(GameState* game, uint8_t p, uint8_t w, uint8_t r, uint8_t answeringPlayer, bool answer, bool updateHintMarks)
{
    ...
    {
        game->positiveAnswers[getVar(cards_getSuspectCard(p), answeringPlayer)] = true;
        game->positiveAnswers[getVar(cards_getWeaponCard(w), answeringPlayer)]  = true;
        game->positiveAnswers[getVar(cards_getRoomCard(r), answeringPlayer)]    = true;
    }
}

void kb_addKeyGuessClauses(GameState* game, uint8_t answeringPlayer, uint8_t card, bool answer)
{
    SAT_Literal cardLit = kb_getLiteral(card, answeringPlayer);
    sat_addUnitClause(game->kb, answer ? cardLit : -cardLit);
}
...
void kb_addStartCardClauses(GameState* game, const GameSettings* settings, uint8_t player, const int8_t* cards)
{
    ...
    {
        SAT_Literal cardLit = kb_getLiteral(cards[i], player);
        sat_addUnitClause(game->kb, cardLit);
    }
}

SAT_Assignment kb_getAssignmentValue(const GameState* game, uint8_t card, uint8_t player)
{
    return game->currAssignment[getVar(card, player)];
}

bool kb_getPositiveAnswerMark(const GameState* game, uint8_t card, uint8_t player)
{
    return game->positiveAnswers[getVar(card, player)];
}
...
bool kb_serializeProblem(const GameState* game, const char* path)
{
    ...
    bool isSerialized = sat_serializeDimacsCnf(game->kb, true, outFile);
