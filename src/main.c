 ...
 * @return Todo
 */
static GameTurnOutcome humanPlayerTurn(const GameSettings* settings, GameState* game, GamePrivateState* roundState);
...
static GameTurnOutcome processGuessResponses(const GameSettings* settings, GameState* game, uint8_t guessingPlayer, uint8_t suspect, uint8_t weapon, uint8_t room)
{
    for(uint8_t i = 1u; i < settings->numPlayers; i++)
    {
        ...
        bool response = parsing_getYesNo("Answer of %s%s (y/n)?> ",
            getPlayerName(settings, i),
            (answerFromKb == SAT_TRUE) ? " (known to be yes)" : (answerFromKb == SAT_FALSE) ? " (known to be no)"
                                                                                            : "");
...
static void setupPlayers(GameSettings* settings)
{
    ...
    while(true)
    {
        ...
        if((settings->playerNames[pl][0] == 'n') || (settings->playerNames[pl][0] == 'N'))
        {
            printf("Not possible.\n");
            free(settings->playerNames[pl]);
            continue;
...
static GameTurnOutcome humanPlayerTurn(const GameSettings* settings, GameState* game, GamePrivateState* roundState)
{
    ...
    uint8_t diceRoll                  = parsing_getDiceRoll("Your dice roll?> ");
...
static GameTurnOutcome otherPlayerTurn(const GameSettings* settings, GameState* game, uint8_t player, GamePrivateState* roundState)
{
    ...
    if(parsing_getYesNo("Is player making accusation (y/n)?> ") == true)
    {
        if(parsing_getYesNo("Is accusation correct (y/n)?> ") == true)
        {
            sheet_print(game, settings);
...
static void gameLoop(const GameSettings* settings, GameState* game, GamePrivateState* roundState)
{
    ...
    while(true)
    {
        ...
        {
            turnOutcome = humanPlayerTurn(settings, game, roundState);
        }
        else
        {
