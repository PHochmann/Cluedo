...
static bool isSatisfiableWithEnvelopeAssumption(const GameState* game, uint8_t p, uint8_t w, uint8_t r)
{
    uint8_t pLit = kb_getLiteral(cards_getSuspectCard(p), ENVELOPE_PLAYER_ID);
    uint8_t wLit = kb_getLiteral(cards_getWeaponCard(w), ENVELOPE_PLAYER_ID);
    uint8_t rLit = kb_getLiteral(cards_getRoomCard(r), ENVELOPE_PLAYER_ID);
    return kb_isSatisfiableWithAssumptions(game, 3u, (SAT_Literal[3]){ pLit, wLit, rLit });
}
